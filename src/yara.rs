...
pub fn yara_total_sources() -> usize {
    let github_repos = get_yara_github_repos();
    let webpage_sources = vec![""];
    let non_empty_pages = webpage_sources.iter().filter(|s| !s.is_empty()).count();
    1 + github_repos.len() + non_empty_pages
}
...
pub fn process_yara(
    ...
    let yara_github_repos = get_yara_github_repos();
...
pub fn get_yara_github_repos() -> Vec<&'static str> {
    vec![
