...
use std::fs;
use crate::download::{download_files_with_progress, download_and_extract_git_repo};
...
pub fn process_suricata_rules(
    ...
    if let Err(e) = fs::create_dir_all(&output_path) {
        eprintln!("Failed to create /suricata directory inside output path: {}", e);
        return;
    }
    ...
    let total_sources = github_repos.len() + webpage_sources.len();

    for (i, repo_url) in github_repos.iter().enumerate() {
        if let Some(cb) = progress_callback.as_mut() {
            cb(i + 1, total_sources, format!("Cloning: {}", repo_url));
        }
    ...
    for (j, url) in webpage_sources.iter().enumerate() {
        ...
        if let Some(cb) = progress_callback.as_mut() {
            cb(index + 1, total_sources, format!("Downloading: {}", url));
        }
